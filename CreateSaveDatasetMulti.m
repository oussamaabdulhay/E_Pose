...
close all
%%
folderPath = 'D:/three_obj/';
fileList = dir(fullfile(folderPath, '*.bag'));
fileNames = {fileList.name};
PC1 = importdata('D:/three_obj/transformed_pcds/dishwasher_filtered/dishwasher.xyz');
PC2 = importdata('D:/three_obj/transformed_pcds/mustard_filtered/mustard.xyz');
PC3 = importdata('D:/three_obj/transformed_pcds/spam_filtered/spam.xyz');
T_obj_r_base(:,:,1) = [-0.10929 0.03984 -0.99137 0.15614;
                        0.98828 0.05435 -0.1076 -0.81268;
                        0.056084 -0.9978 -0.05085 0.0483;
                        0 0 0 1];
T_obj_r_base(:,:,2) = [0.81628 -0.38716 -0.43504 0.033917;
                      -0.40434 0.15337 -0.89925 -0.6623;
                       0.4215 0.9072 -0.03131 0.050155;
                       0 0 0 1];
T_obj_r_base(:,:,3) = [-0.9996 -0.00164 0.0510 0.0235;
                       -0.00418 0.9980 0.04377 -0.8095;
                       -0.55699 0.04252 -0.99703 0.07241;
                       0 0 0 1];


obj1_name = 'dishwasher';
obj2_name = 'mustard';
obj3_name = 'spam';
outputDir = 'D:/EYCB/three_obj';
cls_index = [3 6 9];

for o = 1:length(fileNames)
subDir = fullfile(outputDir, fileNames{o}(1:end-4));
three_obj_file = fullfile(subDir, 'three_obj.txt');
o
%%
...
dvs_events_msgs = readMessages(dvs_events_topic,'DataFormat','struct'); 
dvs_pose_topic = select(bag, 'Topic', '/dvs/pose', ...);%,...
     'Time',[dvs_events_topic.StartTime dvs_events_topic.EndTime]); 
dvs_pose = readMessages(dvs_pose_topic,'DataFormat','struct'); 
...
%%  
T_obj_r_base(:,:,1) = [0.7503 0.1315 -0.64898 0.1507;
                  -0.6278 0.45897 -0.6317 -0.6505;
                  0.2149 0.8755 0.4338 0.0393;
                  0 0 0 1];
T_obj_r_base(:,:,2) = [0.9080 0.01204 0.42366  0.01028;
                   0.4244 -0.0668 -0.9016 -0.7212;
                  0.01735 0.9942 -0.06155 -0.0116;
                  0 0 0 1];
T_obj_r_base(:,:,3) = [0.967 0.2408 -0.01626 -0.02116;
                  -0.240 0.9668 0.0367 -0.7954;
                  0.0245 -0.03163 0.99919 0.03169;
                  0 0 0 1];
rotm = quat2rotm(qi);
...
allData{j,7} = three_channel_image;
...
allData{j,8} = pointsWithTheirProjection;
...
worldPoints = [cam_point1 cam_point2 cam_point3 cam_point4 cam_point5 cam_point6 cam_point7 cam_point8 cam_point9];
pointsWithTheirProjection = [bbox_points_undistorted worldPoints(1:3,:)'];
allData{j,9} = pointsWithTheirProjection;
...
allData{j,11} = allData{j,2}.*mask_obj_1;
allData{j,12} = allData{j,2}.*mask_obj_2;
allData{j,13} = allData{j,2}.*mask_obj_3;
allData{j,14} = masked_image;
allData{j,15} = mask_obj_1;
allData{j,16} = mask_obj_2;
allData{j,17} = mask_obj_3;
allData{j,18} = intrinsicMatrix;
allData{j,19} = T_dvs_r_base(1:3,1:4,2);
end
...
for i=1:length(allData)-6
    i
    bboxData_obj1 = allData{i,8}(1:4,1:2); % Assuming bbox data is in the first column
    bboxDataRow_obj1 = bboxData_obj1';
    bboxDataRow_obj1 = bboxDataRow_obj1(:)';
    bboxData_obj2 = allData{i,9}(1:4,1:2); % Assuming bbox data is in the first column
    bboxDataRow_obj2 = bboxData_obj2';
    bboxDataRow_obj2 = bboxDataRow_obj2(:)';
    bboxData_obj3 = allData{i,10}(1:4,1:2); % Assuming bbox data is in the first column
    ...
    imwrite(allData{i,7}, threeChannelImageFile);

    segmentedImageFile = fullfile(subDir, sprintf('%04d_segmented_image_obj1.png', i));
    imwrite(allData{i,11}, segmentedImageFile);

    segmentedImageFile = fullfile(subDir, sprintf('%04d_segmented_image_obj2.png', i));
    imwrite(allData{i,12}, segmentedImageFile);

    segmentedImageFile = fullfile(subDir, sprintf('%04d_segmented_image_obj3.png', i));
    imwrite(allData{i,13}, segmentedImageFile);

    fprintf(pathsFileID, '%s\n', threeChannelImageFile);

    centerInfo = [allData{i,8}(9,1:2) allData{i,9}(9,1:2) allData{i,10}(9,1:2)]; 
    pose = [allData{i,3} allData{i,4} allData{i,5}] ; 
    intrinsicMatrix = allData{i,18}; 
    cameraPose = allData{i,19}; 
    time = allData{i,6}; 
    keypointprojections =[allData{i,8}(1:8,:) allData{i,9}(1:8,:) allData{i,10}(1:8,:)]; 
    events =allData{i,1};
...
fclose(fileID);
clearvars -except fileNames folderPath cls_index T_obj_r_base fileList outputDir PC1 PC2 PC3 obj1_name obj2_name obj3_name
end
%%
